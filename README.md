...
- **Formulář pro poptávku prostor** – návštěvník vyplní kontaktní údaje, zvolí termín (aplikace kontroluje dostupnost), objekt a počet osob, a odešle žádost čekající na schválení správcem.
  - Po odeslání přijde žadateli e-mail s potvrzením a **jedinečným odkazem pro správu rezervace**. Přes něj může sledovat stav; žádost ani termín přes tento odkaz měnit nelze.
  - Po potvrzení nebo zamítnutí přijde automatický e-mail s výsledkem a cenou.
  - Je-li zapnuto **vzdušné (ubytovací poplatek)**, je žadatel po potvrzení vyzván k vyplnění jmen a dat narození ubytovaných. Upomínky se odesílají 7 dní a 1 den před pobytem.
...
- Nastavit opakující se rezervaci jedním kliknutím – systém automaticky přeskočí státní svátky a prázdniny (lze vypnout). Série se v přehledu zobrazí sbalená pod jedno označení.
...
- Cena je automaticky vypočtena podle ceníku (s respektováním minimální ceny); správce ji může individuálně upravit v detailu rezervace.
...
- **Ceník pronájmu objektů** – ceny za nocleh a pobyt. Při zapnutém vzdušném se nastaví sazby podle věkových kategorií; podporuje kategorii neplatících (např. dospělý doprovod dle vyhlášky).
